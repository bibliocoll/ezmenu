 ...
 * @returns {(event: InputEvent) => boolean} ezjumpUrlHandler with a CSS class to toggle set
 */
function configureURLHandler(validCSSClass: string): (event: InputEvent) => boolean {
    /**
     ...
     */
    return function ezjumpUrlHandler(event: InputEvent): boolean {
 ...
 * @returns {(event: SubmitEvent) => boolean} ezjumpUrlHandler with a CSS class to toggle set
 */
function configureFormHandler(textInputId: string): (event: SubmitEvent) => boolean {
    /**
     * Catch an onsubmit event and reroute the browser to the EZProxy "entry URL"
     * @param {SubmitEvent} event an onsubmit event
     * @returns {boolean} event handler standard return
     */
    return function ezjumpFormHandler(event: SubmitEvent): boolean {
 ...
 * @returns undefined
 */
function registerEzJumpHandlers(ezjumpForm: string = 'ezjumpForm', ezjumpURL: string = 'ezjumpURL', validCSSClass: string = 'valid'): void {
